...
csv_file = "PBI - redcap/raw/raw_demographics.csv"
...
def determine_field_type(row):
    ...
    if data_type == 'Alphanumeric':
        ...
        return 'text'
    elif data_type == 'Date or Date & Time':
        return 'text'
    else:
        return ''
...
redcap_df['Text Validation Type OR Show Slider Number'] = np.where(
    ...
    np.where(df['Data Type'] == 'Date or Date & Time', 'datetime_seconds_ymd', '')
)
...
# Create the first row for record_1
first_row = pd.Series({col: '' for col in redcap_df.columns})
first_row['Variable / Field Name'] = 'record_1'
first_row['Form Name'] = 'demographics'
first_row['Field Type'] = 'text'
first_row['Field Label'] = 'Record ID'
redcap_df = pd.concat([pd.DataFrame([first_row]), redcap_df], ignore_index=True)
...
redcap_df.to_csv("PBI - redcap/_final/demographics.csv", index=False)
